...
import { Asset, AssetProfile } from '../../types';
...
const schema = yup.object({
  ...
  assetProfileId: yup.string().required('Asset profile is required'),
  type: yup.string().required('Type is required'),
});

type FormData = Omit<Asset, 'id' | 'createdAt' | 'updatedAt' | 'assetProfile' | 'parentAsset' | 'attributes' | 'additionalInfo'>;
...
export const AssetForm: React.FC<AssetFormProps> = ({
  ...
  const selectedProfileId = watch('assetProfileId');
  const selectedProfile = profiles.find(p => p.id === selectedProfileId);

  useEffect(() => {
    if (selectedProfile && !initialData) {
      // Auto-fill type based on selected profile
      reset(prev => ({ ...prev, type: selectedProfile.type }));
    }
  ...
  return (
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          <label htmlFor="assetProfileId" className="block text-sm font-medium text-gray-700 mb-1">
            Asset Profile *
          </label>
          <select
            id="assetProfileId"
            {...register('assetProfileId')}
            className={`w-full px-3 py-2 border rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors ${
              errors.assetProfileId ? 'border-red-300 bg-red-50' : 'border-gray-300'
            }`}
            disabled={profilesLoading}
          >
            <option value="">Select asset profile</option>
          ...
          {errors.assetProfileId && (
            <p className="mt-1 text-sm text-red-600">{errors.assetProfileId.message}</p>
          )}
          {profilesLoading && (
      ...
      {selectedProfile && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <h4 className="text-sm font-medium text-blue-900 mb-2">Selected Profile Information</h4>
          <div className="text-sm text-blue-800">
            ...
            <p><strong>Model:</strong> {selectedProfile.model}</p>
          </div>
        </div>
      )}
